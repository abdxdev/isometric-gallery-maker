...
export default function Client() {
  ...
  }, [controls.gradientKey]);

  const noiseDataUrl = useMemo(() => {
    if (!controls.noiseEnabled) return null;
    return generateNoiseDataUrl({ size: 128, opacity: controls.noiseTexture, monochrome: true });
  }, [controls.noiseEnabled, controls.noiseTexture]);

  useEffect(() => {
    setControls((c) => {
      const desired = c.gradientEnabled && c.gradientScope === "frame" ? CONTROL_DEFAULTS.backgroundMargin : 0;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (doc?.documentElement) doc.documentElement.style.zoom = String(controls.pageZoom);
    } catch {}
  }, [controls.pageZoom, canInnerZoom]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    autoCenterTimeoutRef.current = setTimeout(() => triggerAutoCenter(), 80);
    return () => autoCenterTimeoutRef.current && clearTimeout(autoCenterTimeoutRef.current);
  }, [viewW, viewH, controls.backgroundMargin, controls.borderThickness, controls.contentShadowEnabled, controls.contentShadowOffsetX, controls.contentShadowOffsetY, controls.contentShadowBlur, controls.pageZoom, triggerAutoCenter]);
  useEffect(() => {
    triggerAutoCenter();
  }, [triggerAutoCenter]);
            ...
            <div className="flex items-center justify-center w-full h-full relative z-10">
              <div className="max-w-full max-h-full min-w-0 min-h-0">
                <div className="relative inline-block w-fit" style={{ padding: controls.backgroundMargin }}>
                  {controls.gradientScope === "frame" && controls.gradientEnabled && (
                    <div
                      className="absolute pointer-events-none"
                      style={{
                        ...gradientStyle,
                        opacity: controls.gradientOpacity,
                        top: 0,
                        left: 0,
